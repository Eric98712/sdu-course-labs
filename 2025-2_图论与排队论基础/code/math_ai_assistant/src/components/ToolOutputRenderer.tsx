...
function processContent(content: string): ProcessResult {
  ...
  while (remaining.length > 0) {
    ...
    if (imgMatch && imgMatch.index! > 0) {
      ...
      const textHtml = renderMarkdown(before);
      if (textHtml.trim()) {
        foundStructure = true;
        blocks.push(
          <div
    ...
    if (sectionMatch) {
      ...
      if (section.includes('错题已记录')) {
        ...
        blocks.push(
          <div
            className="text-sm text-math-text leading-relaxed [&_.katex]:text-base prose-custom"
            dangerouslySetInnerHTML={{ __html: renderMarkdown(sectionContent) }}
          />
        );
      }
    ...
    // 3) No structure detected → plain text with LaTeX + Markdown
...
/**
 * Render text with LaTeX formulas, then pass through Markdown.
 */
function renderMarkdown(text: string): string {
  // Step 1: render LaTeX $$...$$ and $...$
  const latexHtml = renderLatex(text);
  // Step 2: pass through marked for Markdown (tables, headings, etc.)
  try {
    return marked.parse(latexHtml, {
      async: false,
      breaks: true,
      gfm: true,
    }) as string;
